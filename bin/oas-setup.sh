...
source /usr/local/lib/oas/functions.sh
...
USERNAME=oas
useradd --system oas --shell /bin/false
...
########## Minimal userinput
ask_question "Server name" "$(hostname --fqdn)"
HOSTNAME=${USERINPUT}

ask_question "Server port" "8000"
SERVER_PORT=${USERINPUT}
SOLR_PORT=$(( ${SERVER_PORT}+1 ))
OWLIM_PORT=$(( ${SERVER_PORT}+2 ))

ask_question "Database path" "/oas"
DATABASE_PATH=${USERINPUT}

ask_question "OAI Admin email-address" ""
...
if [ ! -d ${DATABASE_PATH}/solr-state ]; then
    cp -r /usr/share/doc/oas/solr-state ${DATABASE_PATH}
...
############### Server Service ####################
createService $USERNAME $GROUPNAME $SERVICEDIR ${PROJECT_NAME} "
...
"
prepareService $SERVICEDIR ${PROJECT_NAME}-dev
############### /Server Service ####################
...
"
prepareService $SERVICEDIR ${PROJECT_NAME}-solr
############### /SOLR service ####################
...
"
prepareService $SERVICEDIR ${PROJECT_NAME}-triplestore
############### /TripleStore service ####################
...
sleep 300
"
prepareService $SERVICEDIR ${PROJECT_NAME}-delete
############### /Delete service ####################
...
sleep 300
"
prepareService $SERVICEDIR ${PROJECT_NAME}-resolve
############### /Resolve service ####################
...
sleep 300
"
prepareService $SERVICEDIR ${PROJECT_NAME}-harvester
############### /Resolve service ####################
